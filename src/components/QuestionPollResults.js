...
const QuestionPollResults = (props) => {
    ...
    optionSelected = question.optionOne.votes.includes(author.id) ? "optionOne" : "optionTwo";
	
	let optionOneWidth = Math.round((question.optionOne.votes.length / totalVotes) * 100),
    optionTwoWidth = Math.round((question.optionTwo.votes.length / totalVotes) * 100);
